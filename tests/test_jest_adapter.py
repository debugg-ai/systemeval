...
class TestJestAdapterInit:
    ...
    def test_init_with_adapter_config(self, tmp_path: Path):
        ...
        assert adapter.project_root == str(tmp_path)
        assert adapter.config_file == "jest.config.js"
        assert adapter.timeout == 120

    def test_init_with_custom_config_file(self, tmp_path: Path):
        """Test initialization with custom config file."""
        adapter = JestAdapter(str(tmp_path), config_file="custom.jest.config.js")
        assert adapter.config_file == "custom.jest.config.js"


class TestJestAdapterBuildCommand:
    """Tests for command building."""

    def test_build_base_command(self, tmp_path: Path):
        """Test building base jest command."""
        adapter = JestAdapter(str(tmp_path))
    ...
    def test_build_command_with_config(self, tmp_path: Path):
        ...
        config_file = tmp_path / "jest.config.js"
        config_file.write_text("module.exports = {}")

        adapter = JestAdapter(str(tmp_path), config_file="jest.config.js")
        with patch.object(adapter, '_get_npx_path', return_value='/usr/bin/npx'):
            cmd = adapter._build_base_command()
...
class TestJestAdapterGetCommand:
    ...
    def test_get_command_without_parallel(self, tmp_path: Path):
        ...
        with patch.object(adapter, '_get_npx_path', return_value='/usr/bin/npx'):
            ...
            assert '--runInBand' in cmd

    def test_get_command_with_parallel(self, tmp_path: Path):
        """Test get_command with parallel enabled (no --runInBand)."""
        adapter = JestAdapter(str(tmp_path))
        with patch.object(adapter, '_get_npx_path', return_value='/usr/bin/npx'):
            cmd = adapter.get_command(parallel=True)
            assert '--runInBand' not in cmd

    def test_get_command_with_verbose(self, tmp_path: Path):
        """Test get_command with verbose enabled."""
        adapter = JestAdapter(str(tmp_path))
        with patch.object(adapter, '_get_npx_path', return_value='/usr/bin/npx'):
            cmd = adapter.get_command(verbose=True)
            assert '--verbose' in cmd


class TestJestAdapterParseResults:
...
class TestJestAdapterValidateEnvironment:
    ...
    def test_validate_environment_finds_common_config(self, tmp_path: Path):
        """Test validation finds common config files."""
        config_file = tmp_path / "jest.config.ts"
        config_file.write_text("export default {}")

        adapter = JestAdapter(str(tmp_path))
        with patch('shutil.which', return_value='/usr/bin/npx'):
            assert adapter.validate_environment() is True
            assert adapter.config_file == "jest.config.ts"

    def test_validate_environment_package_json_jest(self, tmp_path: Path):
        """Test validation passes when jest config in package.json."""
    ...
    def test_validate_environment_no_config_still_passes(self, tmp_path: Path):
        """Test validation passes even without config (Jest defaults)."""
        adapter = JestAdapter(str(tmp_path))
        with patch('shutil.which', return_value='/usr/bin/npx'):
            # Jest can work without explicit config
            assert adapter.validate_environment() is True


class TestJestAdapterGetAvailableMarkers:
    ...
    def test_get_available_markers(self, tmp_path: Path):
        ...
        assert "jest" in markers
        assert "unit" in markers
        assert "integration" in markers
        assert "snapshot" in markers


class TestJestAdapterDiscover:
    ...
    def test_discover_parses_json_output(self, tmp_path: Path):
        ...
        assert len(tests) == 2
        assert tests[0].name == "math.test.ts"
        assert tests[0].path == "/path/to/math.test.ts"


class TestJestAdapterExecute:
    ...
    def test_execute_handles_timeout(self, tmp_path: Path):
        ...
        import subprocess
        adapter = JestAdapter(str(tmp_path), timeout=1)
...
class TestJestAdapterIntegration:
    """Integration tests for JestAdapter with AdapterConfig."""
    ...
    def test_adapter_config_extra_settings(self, tmp_path: Path):
        ...
        config = AdapterConfig(
            ...
            parallel=True,
            extra={
                "config_file": "custom.jest.config.js",
            }
        )
    ...
    def test_adapter_registered_in_registry(self):
        """Test JestAdapter is registered in the global registry."""
        from systemeval.adapters import list_adapters

        adapters = list_adapters()
        assert "jest" in adapters

    def test_adapter_can_be_retrieved_from_registry(self, tmp_path: Path):
        """Test JestAdapter can be retrieved from registry."""
        from systemeval.adapters import get_adapter

        adapter = get_adapter("jest", str(tmp_path))
        assert isinstance(adapter, JestAdapter)
