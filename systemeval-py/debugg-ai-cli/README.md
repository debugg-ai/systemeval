...
Persist your preferred token and base URL so other commands can reuse them.

```bash
debugg-ai login --token ${YOUR_API_TOKEN} --token-type token --base-url https://localhost:8002
```

The CLI writes the credentials to `~/.debuggai-config.json`; override them later with `--api-key`, `--token`, `--jwt-token`, or `--token-type`.

### `debugg-ai e2e ...`

Wraps `/api/e2e-tests/` with list, create, and status helpers:

```bash
debugg-ai e2e list --project <uuid> --status pending --json
debugg-ai e2e create \
  --description "Homepage smoke test" \
  --project <uuid> \
  --branch main \
  --file-path /app/page.tsx \
  --local-port 3000 \
  --repo-path $(pwd)
debugg-ai e2e status <test-id>
```

Use `--json` for scripts and `--dev` to enable verbose logging. You can also pass `--jwt-token` to switch to `Bearer` auth when the API expects JWTs.

### `debugg-ai crawl list`

Query `/api/v1/graphs/crawl-sessions/` to understand navigation crawls:

```bash
debugg-ai crawl list --project <uuid> --completed true --page 1 --limit 20
```

Filters include `--graph`, `--crawler`, and `--completed` (mirrors `CrawlSessionViewSet` filters). The `--json` flag returns the raw response for further automation.

## TypeScript API

## Design Requirements

- Avoid magic strings/numbers in the CLI or services; centralize values in config, env vars, or constants.
- Keep modules focused—split files once they grow beyond ~600 lines, and avoid sprawling functions so parsing, transport, and orchestration stay readable.
- Enforce separation of concerns: commands, API transport, telemetry/logging, and CLI helpers should not live in the same file.
- Note any justified deviation from these guidelines in comments or docs so future contributors understand the rationale.
...
- Run the `systemeval-next-sample` project in this repo (`npm install && npm run dev`) and exercise the CLI against `http://localhost:3000`.
- Follow `systemeval-next-sample/INTEGRATION.md` for end-to-end steps, including how to point `debugg-ai e2e` and `debugg-ai crawl` at the sentinel APIs described in `docs/crawl-e2e-api-reference.md`.

## Design Requirements

- Do not hard-code API endpoints, secrets, or numeric timeouts; prefer configuration files, environment variables, or constants so the CLI can adapt to different environments.
- Keep modules short and focused; split files that exceed ~600 lines into smaller helpers so contributors can trace the flow.
- Maintain a clear separation between CLI parsing, API transport, git analysis, and orchestration logic.
- Document any deliberate deviation from these rules (e.g., temporary hacks or compatibility workarounds) so future contributors understand the trade-off.
- Refer to `../docs/crawl-e2e-api-reference.md` before extending this CLI with new crawl or E2E calls to reuse the documented shapes and auth requirements.
