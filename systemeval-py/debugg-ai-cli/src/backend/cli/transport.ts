...
export class CLITransport {
    ...
    private readonly authScheme: 'Token' | 'Bearer';
    private readonly expectedPrefix: string;

    constructor({ baseUrl, apiKey, tokenType, timeout, instance }: CLITransportOptions) {
        this.authScheme = tokenType === 'bearer' ? 'Bearer' : 'Token';
        this.expectedPrefix = `${this.authScheme} `;
        ...
        this.setAuthorizationHeader(`${this.authScheme} ${apiKey}`);
        ...
        this.axios.interceptors.request.use((cfg) => {
            ...
            if (authHeader && typeof authHeader === 'string') {
                if (!authHeader.startsWith(this.expectedPrefix)) {
                    log.warn(`Authorization header doesn't start with '${this.expectedPrefix}'`);
                }
                const token = authHeader.replace(this.expectedPrefix, '');
                if (token.length < 10) {
    ...
    updateApiKey(apiKey: string): void {
        ...
        if (this.axios) {
            const headerValue = `${this.authScheme} ${apiKey}`;
            this.setAuthorizationHeader(headerValue);
    ...
    private setAuthorizationHeader(value: string): void {
