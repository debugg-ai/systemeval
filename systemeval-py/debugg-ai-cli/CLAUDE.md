...
- Do not hard-code API endpoints, secrets, or numeric timeouts—prefer configuration, constants, or env vars.
- Keep modules small; split any file exceeding ~600 lines and avoid monstrous functions so new contributors can understand the flow.
- Separate concerns: CLI parsing, API transport, git analysis, and test orchestration should live in different files.
- If you intentionally violate one of these rules, leave a comment explaining the trade-off.
- Check `../docs/crawl-e2e-api-reference.md` before adding new crawl/E2E commands so you reuse the documented endpoint shapes and auth expectations.
