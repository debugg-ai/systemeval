...
- **Integration Testing Infrastructure**
  - `MockDebuggAIServer` - Local HTTP server simulating DebuggAI API for testing
  - `GitRepoFixture` - Temporary git repository management for testing
  - `E2ETestHarness` - Combined test environment orchestrator
  - 275+ integration tests for debugg-ai CLI

- **GitHub Actions CI/CD**
  - `ci.yml` - Automated testing on push/PR (Python 3.10-3.12, TypeScript)
  - `release.yml` - Automated deployment to PyPI and npm on version tags
  - Support for workflow_dispatch manual releases

- **Integration Test Coverage**
  - Basic CLI flow tests (test command, help, version)
  - PR sequence mode tests (--pr-sequence, --base-branch, --head-branch)
  - Wait-for-server tests (--wait-for-server, --server-port, --server-timeout)
  - Artifact download tests (--output-dir, --download-artifacts)
  - Error handling tests (auth errors, API errors, timeouts)
  - CI/CD environment variable detection (GITHUB_SHA, GITHUB_REF_NAME)
  - Systemeval debuggai provider integration
  - CLI subprocess invocation tests
  - Programmatic API tests

### Changed

- Total test count increased from ~1900 to 2158 tests
