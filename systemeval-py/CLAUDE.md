...
- Avoid magic strings/numbers; prefer configuration files or constants so logic can adapt without code changes.
- Keep files and functions focused—if you see a module creeping past 600 lines or functions longer than a page, consider splitting it into smaller, testable pieces.
- Clearly separate parsing, orchestration, and runtime helpers so each layer can evolve independently.
- Document any intentional deviation from these principles to maintain clarity for future reviewers.
- Consult `../docs/crawl-e2e-api-reference.md` before adding new crawl or E2E flows so you reuse the documented endpoint shapes and authentication expectations.
