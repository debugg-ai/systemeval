...
""",
    ...
    # =========================================================================
    # These templates are designed for E2E test generation results from
    # providers like DebuggAI Surfer.

    # -------------------------------------------------------------------------
    # E2E_SUMMARY: One-line summary for E2E generation
    # -------------------------------------------------------------------------
    "e2e_summary": """\
[{{ verdict }}] E2E Generation | {{ tests_generated }} tests generated | {{ duration | round(1) }}s{% if error %} | Error: {{ error | truncate(50) }}{% endif %}
""",

    # -------------------------------------------------------------------------
    # E2E_TABLE: Detailed table for E2E generation results
    # -------------------------------------------------------------------------
    "e2e_table": """\
================================================================================
...
CHANGESET
--------------------------------------------------------------------------------
Base:       {{ changeset.base_ref }}
Head:       {{ changeset.head_ref }}
Changes:    {{ changeset.total_changes }} files (+{{ changeset.total_additions }}/-{{ changeset.total_deletions }})
--------------------------------------------------------------------------------
GENERATION
--------------------------------------------------------------------------------
Tests Generated:  {{ tests_generated }}
Files Generated:  {{ files_generated }}
{% if artifacts %}Output Directory: {{ artifacts.output_directory }}
{% endif %}{% if timed_out %}WARNING: Generation timed out!
{% endif %}
--------------------------------------------------------------------------------
{% if error %}
ERROR
...
""",
    ...
    # -------------------------------------------------------------------------
    # E2E_CI: Structured CI format for E2E generation
    # -------------------------------------------------------------------------
    "e2e_ci": """\
================================================================================
SYSTEMEVAL E2E GENERATION RESULTS
================================================================================
Verdict:     {{ verdict }}
Run ID:      {{ run_id }}
Provider:    {{ provider }}
...
Tests Generated: {{ tests_generated }}
Files Generated: {{ files_generated }}
Timed Out:       {{ 'Yes' if timed_out else 'No' }}
--------------------------------------------------------------------------------
CHANGESET
--------------------------------------------------------------------------------
Base Ref:    {{ changeset.base_ref }}
Head Ref:    {{ changeset.head_ref }}
Files:       {{ changeset.total_changes }}
Additions:   +{{ changeset.total_additions }}
Deletions:   -{{ changeset.total_deletions }}
--------------------------------------------------------------------------------
{% if error %}
ERROR
--------------------------------------------------------------------------------
...
""",
    ...
    # -------------------------------------------------------------------------
    # E2E_GITHUB: GitHub Actions annotation format for E2E
    # -------------------------------------------------------------------------
    "e2e_github": """\
{% if verdict == "PASS" %}::notice::{{ verdict_emoji }} E2E generation succeeded: {{ tests_generated }} tests generated ({{ run_id }})
{% elif verdict == "FAIL" %}::error::{{ verdict_emoji }} E2E generation failed: {{ error | truncate(100) }} ({{ run_id }})
{% else %}::error::{{ verdict_emoji }} E2E generation error: {{ error | truncate(100) | default('Unknown error') }} ({{ run_id }})
{% endif %}
{% if timed_out %}::warning::E2E generation timed out after {{ duration | round(0) }} seconds
{% endif %}
{% for warning in warnings %}::warning::{{ warning | truncate(100) }}
{% endfor %}
""",

    # -------------------------------------------------------------------------
    # E2E_MARKDOWN: Full markdown report for E2E generation
    # -------------------------------------------------------------------------
    "e2e_markdown": """\
# E2E Test Generation Results
...
{% else %}
**E2E test generation error.** Check configuration and provider status.
{% endif %}
...
| Base Ref | `{{ changeset.base_ref }}` |
| Head Ref | `{{ changeset.head_ref }}` |
| Files Changed | {{ changeset.total_changes }} |
| Additions | +{{ changeset.total_additions }} |
| Deletions | -{{ changeset.total_deletions }} |
...
| Metric | Value |
|--------|-------|
| Output Directory | `{{ artifacts.output_directory }}` |
| Total Tests | {{ artifacts.total_tests }} |
| Total Files | {{ artifacts.test_files | length }} |

{% if artifacts.test_files %}
### Generated Files

{% for file in artifacts.test_files[:10] %}
- `{{ file }}`
{% endfor %}
{% if artifacts.test_files | length > 10 %}
_...and {{ artifacts.test_files | length - 10 }} more files_
{% endif %}
{% endif %}
{% endif %}
...
{% if timed_out %}
> **Warning:** Generation timed out after {{ duration | round(0) }} seconds.
{% endif %}
""",

    # -------------------------------------------------------------------------
    # E2E_SLACK: Slack message format for E2E generation
    # -------------------------------------------------------------------------
    "e2e_slack": """\
{{ verdict_emoji }} *{{ verdict }}* | `E2E Generation`
>*Run:* {{ run_id }} | *Provider:* {{ provider }} | *Duration:* {{ duration | round(1) }}s
>*Tests Generated:* {{ tests_generated }} | *Files:* {{ files_generated }}
{% if changeset %}*Changeset:* {{ changeset.total_changes }} files (+{{ changeset.total_additions }}/-{{ changeset.total_deletions }}){% endif %}
{% if error %}
